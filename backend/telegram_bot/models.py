...
class TgChat(models.Model):
    ...
    region = models.CharField(max_length=100, verbose_name="Country or city user lives in", null=True)
...
class TgMessage(models.Model):
    ...
    message_type = models.CharField(max_length=50, choices=MessageTypeChoices.choices, default=MessageTypeChoices.TEXT)

    class Meta:
        ordering = ['-created_at']

    def __str__(self):
        return f"{self.message_from.capitalize()} - {self.tg_chat_id}: {self.message}"
